...
#ifndef DEBUGFS_EXTEND_GPIO_PINS

#define MAX_GPIO_PIN		132
#define MAX_IFACE_NAME_LEN	8	/* SHOULD BE NULL TERMINATED */
#define IFACE_PREFIX		"gpio"
...
static struct gpio_desc gpio_table[MAX_GPIO_PIN+1];


static int gpios_set(void *data, u64 val)
{
	struct gpio_desc *gpio = data;
        gpio_set_value(gpio->index, val);
        return 0;
}
static int gpios_get(void *data, u64 *val)
{
	struct gpio_desc *gpio = data;
        *val=gpio_get_value(gpio->index);
        return 0;
}


DEFINE_SIMPLE_ATTRIBUTE(gpio_pin_fops, gpios_get, gpios_set, "%llu\n");

#define GEN_GPIO_TABLE(_tbl, min, max, prefix) 		\
do {							\
  int i;						\
  for(i=min; i<=max; ++i) { 				\
    snprintf(_tbl[i-min].name, MAX_IFACE_NAME_LEN,  	\
             prefix "%d", i );				\
    _tbl[i-min].index=i;			      }	\
}while(0);


#define CREATE_GPIO_FILES(_tbl, _mode, _ent, _fops)	\
do {							\
  int i;						\
  for(i=0; i<ARRAY_SIZE(_tbl); ++i )			\
    debugfs_create_file(_tbl[i].name, _mode, _ent, &_tbl[i], _fops); \
}while(0);


#endif /* DEBUGFS_EXTEND_GPIO_PINS */
#endif
...
#ifdef CONFIG_MACH_MSM7X25_QRD
#ifndef DEBUGFS_EXTEND_GPIO_PINS
	GEN_GPIO_TABLE(gpio_table, 0, 132, IFACE_PREFIX);
	CREATE_GPIO_FILES(gpio_table, 0644, dent, &gpio_pin_fops);
#endif /* DEBUGFS_EXTEND_GPIO_PINS */
#endif
