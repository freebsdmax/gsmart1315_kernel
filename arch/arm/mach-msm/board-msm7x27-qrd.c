			     ...
			     char *buf)
{
	return sprintf(buf,"%s:%s:%s:%s",
		"0x01:139:40:460:80:40",
		"0x01:102:120:460:80:40",
		"0x01:158:200:460:80:40",
		"0x01:217:280:460:80:40");
}
...
int msm7x25_lcd_bl(int level)
{
	...
	if(level)
        ...
        msm_proc_comm(PCOM_VREG_SWITCH, &vreg, &on);
...
#define SPI_SDI     102
#define SPI_SDO     103
#define LCD_RESET   101
#define SPI_CS      131
#define SPI_CLK     132
#define LCD_TE      124

static int lcdc_gpio_array_num_evt1[] = {
        SPI_SDI, /* spi_sdi */
        SPI_SDO, /* spi_sdo */
        LCD_RESET, /* lcd_reset */
        SPI_CS, /* spi_cs  */
...
static uint32_t lcdc_gpio_table_evt1[] = {
...
{
        if (gpio_request(lcdc_gpio_array_num_evt1[0], "spi_sdi"))
                pr_err("failed to request gpio spi_sdi\n");
        if (gpio_request(lcdc_gpio_array_num_evt1[1], "spi_sdo"))
                pr_err("failed to request gpio spi_sdo\n");
        if (gpio_request(lcdc_gpio_array_num_evt1[2], "lcd_reset"))
                pr_err("failed to request gpio lcd_reset\n");
        if (gpio_request(lcdc_gpio_array_num_evt1[3], "spi_cs"))
                pr_err("failed to request gpio spi_cs\n");
        if (gpio_request(lcdc_gpio_array_num_evt1[4], "gpio_clk"))
                pr_err("failed to request gpio_clk\n");
        if (gpio_request(lcdc_gpio_array_num_evt1[5], "lcd_te"))
                pr_err("failed to request lcd_te\n");
}

static void lcdc_config_gpios_evt1(int enable)
{
        int n;
        for (n = 0; n < ARRAY_SIZE(lcdc_gpio_table_evt1); n++)
                gpio_tlmm_config(lcdc_gpio_table_evt1[n], enable);
}

static struct msm_panel_common_pdata lcdc_panel_data_evt1 = {
        .panel_config_gpio = lcdc_config_gpios_evt1,
        .gpio_num          = lcdc_gpio_array_num_evt1,
        .pmic_backlight    = msm7x25_lcd_bl,
};

static struct platform_device lcdc_device_evt1 = {
        .name   = "lcdc_device",
        .id     = 1,
        .dev    = {
                .platform_data = &lcdc_panel_data_evt1,
        }
};
...
int GBC_BTS4025_Power(int on)
{
	...
	if (on) {
		...
		printk(KERN_INFO "[vin][%d]GPIO_BTSYSRST(gpio16) on ok\r\n", __LINE__);
...
int GBC_BTS4020BD_Power(int on)
{
	...
	if (on) {
		...
		printk(KERN_INFO "[vin][%d]: mpp3 on ok\r\n", __LINE__);
		...
		printk(KERN_INFO "[vin][%d]: mpp3 off ok\r\n", __LINE__);
...
#ifndef CONFIG_MACH_MSM7X27_QRD
...
#ifdef CONFIG_I2C_SI470X
static unsigned fm_config_power_on[] = {
	GPIO_CFG(123, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),	/* LCD_RESET */
};

static void fm_power(void) {
	gpio_tlmm_config(fm_config_power_on[0], GPIO_ENABLE);
	gpio_set_value(123, 1);
...
static void __init msm7x2x_init_mmc(void)
{
	...
	if (machine_is_msm7x25_surf() || machine_is_msm7x27_surf() ||
		machine_is_msm7x27_ffa() || machine_is_msm7x27_qrd()) {
#ifdef CONFIG_MMC_MSM_SDC2_SUPPORT
...
#define IR_LED_GPIO 82
static unsigned irled_config_power_on[] = {
	GPIO_CFG(93, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA),
};
static int irled_power()
{
	int err;
	err = gpio_tlmm_config(irled_config_power_on[0], GPIO_ENABLE);
	if (err) {
		printk(KERN_ERR "%s: gpio_tlmm_config failed\n", __func__);
		return err;
	}
	gpio_set_value(IR_LED_GPIO, 1); /* 0:LOW  1:HIGH */
...
static void __init msm7x2x_init(void)
{
	...
	uint32_t            ctp_gpio_table[] = {
		GPIO_CFG(93, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),
	};
	int		ret = 0;
	#endif
	if (socinfo_init() < 0)
...
MACHINE_START(MSM7X27_QRD, "QCT MSM7x27 QRD")
