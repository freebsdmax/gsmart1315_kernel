...
#if defined(CONFIG_QRD_GPIO_KEYPAD)
#include "keypad-qrd.h"
#endif
#endif
...
#if defined(CONFIG_MACH_MSM7X25_QRD)
#define HS_STEREO_HEADSET_K	0x7E
#else
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	printk("%s:[HANDSET] key_code = %d, key_parm = %d\n",__func__, key_code, key_parm);
