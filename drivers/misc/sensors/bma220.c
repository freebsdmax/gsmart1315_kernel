...
#define BMA220_NOISE_REDUCTION 1
...
int bma220_set_mode(unsigned char mode) 
{
	...
	switch(p_bma220->mode) {
		case 0: if (mode==1)
				    ...
				    comres = bma220_set_suspend(&data);
				break;
		case 1: comres = bma220_set_sleep_en(0);
				if (mode==2)
					comres |= bma220_set_suspend(&data);
				break;
		case 2: comres = bma220_set_suspend(&data);
				if (mode==1)
					comres |= bma220_set_sleep_en(1);
				break;
...
*/
int bma220_set_suspend(unsigned char *data)
{
	int comres;
	if (p_bma220==0) 
		return E_BMA_NULL_PTR;
...
/** Get Acceleration Data for all 3 axis XYZ
	...
	\return result of communication routines
*/
signed char x1, x2, y1, y2, z1, z2, sx, sy, sz;
signed char buf[3] = {0x99, 0x99, 0x99};
...
#ifdef BMA220_NOISE_REDUCTION
	{
		int i;
		if (buf[0] == (signed char)0x99) {/* set all data the same */
			buf[0] = accel->x;
			buf[1] = accel->y;
			buf[2] = accel->z;
		} else {
			if (((int)buf[0] - (int)accel->x) > 1)
				buf[0] = accel->x + 1;
			if (((int)buf[0] - (int)accel->x) < -1)
				buf[0] = accel->x - 1;

			if (((int)buf[1] - (int)accel->y) > 1)
				buf[1] = accel->y + 1;
			if (((int)buf[1] - (int)accel->y) < -1)
				buf[1] = accel->y - 1;

			if (((int)buf[2] - (int)accel->z) > 1)
				buf[2] = accel->z + 1;
			if (((int)buf[2] - (int)accel->z) < -1)
				buf[2] = accel->z - 1;
		}
