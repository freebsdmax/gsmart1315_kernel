		...
		struct msm_frame *frame)
{
	...
	struct msm_vfe_phy_info *pphy;
        ...
        if(!sync) {
		pr_err("%s: no msm_sync structure.\n", __func__);
                return -EAGAIN;
	}
...
//	#ifdef CAMERA_AF
       ...
       switch (udata.type) {
                ...
                case CAMERA_SET_PARM_AUTO_FOCUS:
                        ...
                        printk("--CAM_msm--CAMERA_SET_PARM_AUTO_FOCUS  rc = %d \n", rc);

                        udata.status = CAMERA_EXIT_CB_DONE;
                        break;

		   case CAMERA_SET_PARM_ANTIBANDING:

			    printk("--CAM_msm--CAMERA_SET_PARM_ANTIBANDING  \n");
	...
	CDBG("%s: rc %d\n", __func__, rc);
//printk(KERN_ERR " msm_control done\n");
