...
const u32 battery_valid_rpc_vers[] = {
...
#define VBATT_RECORD_VOLTAGE_ITEMS		16
#define VBATT_RECORD_VOLTAGE_SHIFT		4
static u32 record_voltages[VBATT_RECORD_VOLTAGE_ITEMS];
...
static void msm_batt_update_psy_status(void)
{
	...
	if (msm_batt_info.battery_voltage != battery_voltage) {
		static u8 cnt = 0, record_started = 0;
		u8 i = 0;
		u32 sum = 0;

		if (!record_started) {
			memset(record_voltages, battery_voltage, VBATT_RECORD_VOLTAGE_ITEMS);
			record_started = 1;
		} else
			record_voltages[cnt] = battery_voltage;

		cnt++;
		if (cnt >= VBATT_RECORD_VOLTAGE_ITEMS)
			cnt = 0;

		for (i = 0; i < VBATT_RECORD_VOLTAGE_ITEMS; i++)
			sum += record_voltages[i];
		battery_voltage = sum >> VBATT_RECORD_VOLTAGE_SHIFT;
...
struct batt_client_registration_req {
	...
	/* The call back data */
	u32 cb_data;
#ifndef CONFIG_MSM_QRD_SUPPORT_MP2220_PLUS_7010
	u32 more_data;
#endif
	u32 batt_error;
};
...
static int msm_pm_vbatt_register(struct msm_rpc_client *clnt,
		u32 desired_batt_voltage, u32 voltage_direction, u32 batt_cb_id,
		u32 cb_data, s32 *handle)
{
	struct batt_client_registration_req arg;
	struct batt_client_registration_rep ret;
	int rc;
	...
	rc = msm_rpc_client_req(msm_batt_info.batt_client,
			...
			msm_batt_register_arg_func, &arg,
			msm_batt_register_ret_func, &ret,
			msecs_to_jiffies(BATT_RPC_TIMEOUT));
...
#ifdef CONFIG_MSM_QRD_SUPPORT_MP2220_PLUS_7010
	/* FIXME for the moment, the code only support new rpc if the macro defined
	 * Give user one warning for this!
	 */
	if (msm_batt_info.batt_api_version != BATTERY_RPC_VER_4_1) {
		printk("ERROR: RPC arguments error!\n" \
			"Try to undefine CONFIG_MSM_QRD_SUPPORT_MP2220_PLUS_7010 macro\n");
		BUG();
	}
#endif
...
static int msm_batt_cleanup(void)
{
	...
	if (msm_batt_info.batt_chg_handle != INVALID_BATT_HANDLE) {

		rc = msm_batt_deregister(msm_batt_info.batt_chg_handle);
		if (rc < 0)
			pr_err("%s: FAIL: msm_batt_deregister. rc=%d\n",
			       __func__, rc);
	}
...
static int __devinit msm_batt_init_rpc(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(battery_valid_rpc_vers); i++) {
		...
		if (msm_batt_info.batt_client == NULL) {
			...
			msm_batt_info.batt_api_version = battery_valid_rpc_vers[i];
			break;
		}
	}
