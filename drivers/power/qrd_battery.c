...
#define BATTERY_CB_ID_LVL_CHG	4

#define BATTERY_LOW            	2800
#define BATTERY_HIGH           	4300

#define ONCRPC_CHG_GET_GENERAL_STATUS_PROC 	12
#define ONCRPC_CHG_GET_GENERAL_STATUS_PROC_1	14
...
#if DEBUG
#define DBG_LIMIT(x...) do {/*if (printk_ratelimit()) pr_debug(x);*/printk(x); } while (0)
#else
...
struct msm_battery_info {
	...
	struct msm_rpc_client *oem_client;
...
static int msm_pm_vbatt_get_battery_level(void)
{
	struct oem_rapi_client_streaming_func_arg arg;
	struct oem_rapi_client_streaming_func_ret ret;
	int input, retval, outlen, rc;

	arg.event = OEM_RAPI_CLINET_GET_BATTERY_LEVEL;
	arg.cb_func = NULL;
	arg.handle = 0;
	...
	rc = oem_rapi_client_streaming_function(msm_batt_info.oem_client,
		&arg,
		&ret);
...
static void msm_update_battery_level(void)
{
	int lvl = msm_pm_vbatt_get_battery_level();

	msm_batt_info.batt_capacity = lvl;
...
{
	int rc ;
#ifdef CONFIG_MSM_QRD_SUPPORT_MP2220_PLUS_7010
	unsigned int proc;
#endif
...
#if defined(CONFIG_MSM_QRD_SUPPORT_MP2220_PLUS_7010) && \
	!defined(CONFIG_MSM_QRD_SUPPORT_MP2220_PLUS_7030)
	if (msm_batt_info.chg_api_version == CHG_RPC_VER_3_1)
		proc = ONCRPC_CHG_GET_GENERAL_STATUS_PROC_1;
	else
		proc = ONCRPC_CHG_GET_GENERAL_STATUS_PROC;
#endif
...
static void msm_batt_update_psy_status(void)
{
	...
	if (!battery_voltage) {
		if (charger_status == CHARGER_STATUS_INVALID) {
			...
			battery_voltage = msm_pm_vbatt_read_mv(
...
void msm_batt_early_suspend(struct early_suspend *h)
{
	...
	if (msm_batt_info.batt_handle != INVALID_BATT_HANDLE) {
		rc = msm_pm_vbatt_modify_client(msm_batt_info.batt_client,
	...
	if (INVALID_BATT_HANDLE != msm_batt_info.batt_lvl_handle) {
	    rc = msm_pm_vbatt_deregister_client(msm_batt_info.batt_client,
		msm_batt_info.batt_lvl_handle);
	    if (0 > rc) {
		pr_err("%s: msm_batt_modify_client. rc=%d\n",
			       __func__, rc);
			return;
	    } else
		msm_batt_info.batt_lvl_handle = INVALID_BATT_HANDLE;
	}
...
void msm_batt_late_resume(struct early_suspend *h)
{
	...
	if (INVALID_BATT_HANDLE == msm_batt_info.batt_lvl_handle) {
	    rc = msm_pm_vbatt_register(msm_batt_info.batt_client,
			BATTERY_LOW,
			VBATT_BATTERY_LEVEL_EVENTS,
			BATTERY_CB_ID_LVL_CHG,
			VBATT_BATTERY_LEVEL_EVENTS,
			&msm_batt_info.batt_lvl_handle);
	    if (0 > rc) {
		pr_err("%s: msm_batt_modify_client FAIL rc=%d\n",
...
static int msm_batt_cleanup(void)
{
	...
	if (INVALID_BATT_HANDLE != msm_batt_info.batt_chg_handle) {
		rc = msm_pm_vbatt_deregister_client(msm_batt_info.batt_client,
			msm_batt_info.batt_chg_handle);
		if (0 > rc)
			pr_err("%s: FAIL msm_pm_vbatt_deregister_client. rc=%d\n",
				__func__, rc);
	}
	msm_batt_info.batt_chg_handle = INVALID_BATT_HANDLE;

	if (msm_batt_info.oem_client)
			    ...
			    void *buffer, int in_size)
{
	...
	u32 procedure;
	u32 accept_status;
	struct vbatt_cb_arg *cb_arg;
	u32 cb_id, cb_data;

	req = (struct rpc_request_hdr *)buffer;
	procedure = be32_to_cpu(req->procedure);

	cb_arg = (struct vbatt_cb_arg *)(req + 1);
	cb_id = be32_to_cpu(cb_arg->cb_id);
	cb_data = be32_to_cpu(cb_arg->cb_data);
	...
	if (accept_status == RPC_ACCEPTSTAT_SUCCESS) {
		switch (cb_data) {
		case BATTERY_ALL_ACTIVITY:
		case VBATT_CHG_EVENTS:
			msm_batt_update_psy_status();
			break;

		case VBATT_BATTERY_LEVEL_EVENTS:
			msm_update_battery_level();
			break;
		default:
			break;
...
static int __devinit msm_batt_probe(struct platform_device *pdev)
{
	...
	rc = msm_pm_vbatt_enable_disable_filter(msm_batt_info.batt_client,
		msm_batt_info.batt_handle, 1, VBATT_FILTER);
	if (rc < 0) {
		dev_err(&pdev->dev,
	...
	if (0 > rc) {
		dev_err(&pdev->dev,
			"%s: msm_pm_vbatt_register failed rc = %d\n",
			__func__, rc);
		msm_batt_cleanup();
		return rc;
	}
	...
	msm_batt_info.oem_client = oem_rapi_client_init();
	if (NULL == msm_batt_info.oem_client) {
		dev_err(&pdev->dev,
			"%s: oem_rapi_client_init failed\n", __func__);
		msm_batt_cleanup();
		return -ENODEV;
	}
...
static int __devinit msm_batt_init_rpc(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(chg_valid_rpc_vers); i++) {
		msm_batt_info.chg_ep =
			msm_rpc_connect_compatible(CHG_RPC_PROG, chg_valid_rpc_vers[i], 0);
		...
		if (msm_batt_info.chg_ep == NULL) {
			...
			msm_batt_info.chg_api_version = chg_valid_rpc_vers[i];
			break;
		}
	}
	...
	msm_batt_info.batt_client = msm_pm_vbatt_init_client("qrd_battery", 1,
		msm_batt_cb_func,
		&msm_batt_info.batt_api_version);
	if (NULL == msm_batt_info.batt_client) {
		pr_err("%s: FAIL: msm_pm_vbatt_init_client.", __func__);
		return -ENODEV;
	}
