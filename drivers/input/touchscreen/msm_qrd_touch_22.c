...
static struct early_suspend	msm_early_suspend;
static int			g_irq;
...
static void msm_early_suspend_func(struct early_suspend *h)
{
	printk("<0> RTP sleeps\n");
	disable_irq(g_irq);
}

static void msm_late_resume_func(struct early_suspend *h)
{
	printk("<0> RTP wakes up\n");
	enable_irq(g_irq);
}
...
static int __devinit ts_probe(struct platform_device *pdev)
{
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, 840, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, VIRT_GAP, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_WIDTH_MAJOR, 0, 15, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, pressure_max,\
			     0, 0);
	...
	msm_early_suspend.level			= EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
	msm_early_suspend.suspend		= msm_early_suspend_func;
	msm_early_suspend.resume		= msm_late_resume_func;
	register_early_suspend(&msm_early_suspend);
