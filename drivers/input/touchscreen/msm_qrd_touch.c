...
#ifdef SUPPORT_VIRTUAL_KEY
int virtual_key_func(struct ts *ts, int x, int y, int pressure)
{
	...
	if (pressure) {
		...
		if ((x > (ts->x_max / 4)) && (x <= (ts->x_max / 2))) {
			input_report_key(ts->input, KEY_HOME, 1);
			ts->virtual_key = 2;
			return 1;
		}

		if ((x > (ts->x_max / 2)) && (x <= (((ts->x_max / 4)) * 3))) {
...
{
	#ifdef SUPPORT_VIRTUAL_KEY
	if ((y > VIRT_GAP) || (ts->virtual_key)) {
		virtual_key_func(ts, x, y, pressure);
		return;
	}
	#endif
...
static int __devinit ts_probe(struct platform_device *pdev)
{
	...
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, pressure_max, 0, 0);
	#else
